...
CODEX_BUILD_PATTERN = re.compile(r"^codex\.[a-z0-9]+(?:-[a-z0-9]+)*$")
...
def validate_versions(versions: dict[str, str]) -> str:
    ...
    codex = normalized.get(".codex-plugin/plugin.json", "")
    if "+" in codex:
        base, build = codex.split("+", 1)
        require(CODEX_BUILD_PATTERN.fullmatch(build) is not None, f"invalid Codex cachebuster: {codex}")
        normalized[".codex-plugin/plugin.json"] = base
    require(len(set(normalized.values())) == 1, f"manifest base versions do not match: {versions}")
    version = next(iter(normalized.values()))
